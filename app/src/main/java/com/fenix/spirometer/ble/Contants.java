package com.fenix.spirometer.ble;

public class Contants {
    public static final int MSG_DEVICE_DISCONNECTED = 0;
    public static final int MSG_DEVICE_CONNECTING = 1;
    public static final int MSG_DEVICE_CONNECTED = 2;
    public static final int MSG_DEVICE_INITIALIZED = 3;
    public static final int MSG_DATA_RECEIVED = 4;
    public static final int MSG_FAILED = 5;
    public static final int MSG_DEVICE_STATE_CHANGE = 6;
    public static final int MSG_DEVICE_DISCONNECTING = 7;
    public static final int MSG_RAW_DATA_RECEIVED = 8;
    public static final int MSG_TESTING = 9;
    public static final int MSG_TEST_FINISH = 10;
    public static final int MSG_COMMAND_START = 11;
    public static final int MSG_COMMAND_STOP = 11;
}
